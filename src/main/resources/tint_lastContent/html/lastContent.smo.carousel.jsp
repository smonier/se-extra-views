...
<template:addResources type="javascript"
                       resources="equalizeCardHeights.js" />
    ...
    <c:otherwise>
                    ...
                    <div class="col-12">
                        <div id="${sliderId}" class="owl-carousel">
                            <c:forEach items="${result.nodes}" var="node">
                                <template:module view="${subNodesView}" node="${node}" editable="false"/>
                            </c:forEach>
                        </div>
                    </div>
                </div>
        ...
        <script>
            $(document).ready(function () {
                const $carousel = $('#${sliderId}'); // make sure sliderId is resolved server-side
